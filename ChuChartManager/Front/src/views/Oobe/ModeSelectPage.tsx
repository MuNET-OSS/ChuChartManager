...
export default defineComponent({
  props: {
    ...
    onComplete: { type: Function as PropType<(opts: { isRemote: boolean; useAuth: boolean; authUsername: string; authPassword: string }) => void>, required: true },
  },
  setup(props) {
    const { t } = useI18n();
    ...
    return () => (
      <div class="flex flex-col items-center justify-center h-full px-12">
        ...
        <TransitionVertical>
          {isRemote.value && (
            <div class="flex flex-col gap-3 w-full max-w-sm my-3">
              <div class="flex items-center gap-2 cursor-pointer" onClick={() => { useAuth.value = !useAuth.value }}>
                <div class={['w-5 h-5 rounded-full border-2 transition-colors shrink-0',
                  useAuth.value ? 'bg-[oklch(0.7_0.15_var(--hue))] border-[oklch(0.7_0.15_var(--hue))]' : 'border-gray-300']} />
                <span>{t('oobe.enableAuth')}</span>
              </div>
              <TransitionVertical>
                {useAuth.value && (
        ...
        <Button onClick={handleComplete} ing={props.completing}>
          {t('oobe.complete')}
        </Button>
      </div>
    );
